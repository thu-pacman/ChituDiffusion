...
def test_warmup_includes_fixed_queryformer_tokens() -> None:
    captured = {}

    def warmup(transformer, **kwargs):
        captured.update(kwargs)
        return {"rows": []}

    runtime = object.__new__(LLaDAImageRuntimeMixin)
    runtime.queryformer = SimpleNamespace(config=SimpleNamespace(num_queries=256))
    runtime.transformer = SimpleNamespace(epe=SimpleNamespace(warmup_transformer=warmup))
    runtime.latent_scale_factor = 16
    runtime.warmup_epe(resolutions=((512, 512),))
    assert captured["text_tokens"] == 288
    runtime.warmup_epe(resolutions=((512, 512),), text_tokens=512)
    assert captured["text_tokens"] == 512
