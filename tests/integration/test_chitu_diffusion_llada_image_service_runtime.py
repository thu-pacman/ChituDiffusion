...
@pytest.mark.parametrize(
    ("vae", "expected_degree"),
    (({}, 1), ({"halo": 4}, 1), ({"degree": None}, 1), ({"enabled": True}, None), ({"degree": 2}, 2)),
)
def test_llada_stage_requires_opt_in_for_approximate_vae(vae, expected_degree):
    config = StageServiceConfig.from_mapping(
        {
            ...
            "parallelism": {
                "vae": vae,
                "scheduler": {"policy": "static_cp"},
            },
            "factory_args": {"model_path": "/models/llada-image"},
        }
    )
    assert config.parallelism.vae.degree == expected_degree
