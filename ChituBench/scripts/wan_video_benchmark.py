...
DEFAULT_PROMPT_FILE = PROJECT_ROOT / "ChituBench" / "prompts" / "wan2_1_t2v_1_3b_attention.json"
...
def _frame_num() -> int:
    return int(os.getenv("CHITUBENCH_FRAME_NUM", "81"))


def _size() -> tuple[int, int]:
    raw = os.getenv("CHITUBENCH_VIDEO_SIZE", os.getenv("CHITUBENCH_IMAGE_SIZE", "832,480"))
...
def build_requests(args: ServeConfig) -> list[DiffusionUserRequest]:
    ...
    negative_prompt = os.getenv("CHITUBENCH_NEGATIVE_PROMPT", DEFAULT_NEGATIVE_PROMPT)
    sample_solver = os.getenv("CHITUBENCH_SAMPLE_SOLVER", "unipc")
    requests = []
...
def run_benchmark(args: ServeConfig, run_context: DiffusionTestRunContext):
    ...
    try:
        if rank == 0:
            ...
            logger.info("ChituBench Wan requests: %s", reqs)
        ...
        if rank == 0:
            logger.info("ChituBench Wan benchmark time cost %.3fs", elapsed_s)
...
def main(args: ServeConfig):
    if not str(args.models.name).startswith("Wan"):
        raise ValueError(f"wan_video_benchmark expects a Wan model, got {args.models.name}")
