...
class EpeZImagePipeline(ZImagePipeline):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs: Any):
        ...
        if transformer is None:
            ...
            transformer.configure_epe(parallel)
        ...
        pipeline._epe_parallel = parallel
        return pipeline

    @property
    def parallel_context(self) -> EpeParallelContext:
        if self._epe_parallel is None:
            raise RuntimeError("pipeline has no EPE parallel context")
        return self._epe_parallel

    @torch.inference_mode()
    def __call__(self, *args: Any, lane_ranks: tuple[int, ...] | None = None, **kwargs: Any):
    ...
    def finalize_request(
        ...
        *,
        ...
        """Decode a completed request with the native Diffusers VAE path."""
    ...
    def close(self) -> None:
        if self._epe_parallel is not None:
            self._epe_parallel.close()
