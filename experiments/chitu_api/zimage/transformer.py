...
from .attention import ZImageAgkvAttnProcessor
from .parallel import EpeParallelContext


class EpeZImageTransformer2DModel(ZImageTransformer2DModel):
    """Diffusers Z-Image transformer with topology-aware AGKV context parallelism."""

    _epe_parallel: EpeParallelContext | None = None
    _epe_attn_processor: ZImageAgkvAttnProcessor | None = None
    ...
    def configure_epe(self, parallel: EpeParallelContext) -> None:
        processor = ZImageAgkvAttnProcessor(parallel)
        for block in [*self.noise_refiner, *self.context_refiner, *self.layers]:
            block.attention.set_processor(processor)
        self._epe_parallel = parallel
    ...
    def forward(
        ...
        if len(x) > 1:
            ...
            return (outputs,) if not return_dict else Transformer2DModelOutput(sample=outputs)
    ...
    def _cp_forward(
        ...
        *,
        ...
        assert parallel is not None and processor is not None
        ...
        image_sequence, image_freqs, image_mask, _, image_noise = self._prepare_sequence(
        ...
        if image_mask is not None or caption_mask is not None:
            raise NotImplementedError("EPE Z-Image CP does not support padded sequences")
        ...
        try:
            ...
            unified, unified_freqs, unified_mask, unified_noise = self._build_unified_sequence(
                image_sequence,
                image_freqs,
                image_lengths,
                None,
