...
def create_app(backend: ServiceBackend) -> FastAPI:
    app = FastAPI(title="Chitu EPE Image Decode Service", version="0.1.0")
    ...
    def request_image(request_id: str) -> Response:
        ...
        if image is None:
            raise HTTPException(status_code=500, detail="completed request has no image")
    ...
    def health() -> HealthResponse:
        ...
        if response.state == "failed":
            raise HTTPException(status_code=503, detail=response.fatal_error or "runtime failed")
        return response
