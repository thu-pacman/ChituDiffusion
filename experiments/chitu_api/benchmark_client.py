...
def _wait_requests(
    ...
    *,
    ...
    if remaining:
        raise TimeoutError(f"requests did not finish within {timeout_s}s: {sorted(remaining)}")
    return statuses
...
def run_benchmark(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for entry in requests:
        ...
        _json_request(args.endpoint, "POST", "/v1/image-decode", _request_payload(entry))
    ...
    first_server_submit = min(float(status["submitted_at"]) for status in statuses.values())
    ...
    for request_id in request_ids:
        ...
        admission_delay_ms = admission["submit_finish_ms"] - admission["trace_arrival_ms"]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Replay a Chitu Poisson trace against the standalone Z-Image API")
