...
from chitu_diffusion.parallel.fast_cp.ulysses_ring import _all_to_all_4d
...
def test_usp_routes_all_to_all_through_selected_transport(monkeypatch):
    calls = []
...
def test_fast_subgroup_transport_uses_shared_runtime(monkeypatch):
    ...
    monkeypatch.setattr(
        FastUlyssesAllToAll,
        "supports",
        staticmethod(lambda tensor, scatter_dim, gather_dim: tensor.dtype == torch.bfloat16),
    )
