...
from chitu_diffusion import cli
...
def test_cli_dispatches_generate_and_serve(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert calls == [
        ("chitu_diffusion.examples.zimage_epe", ["--prompt", "test"]),
        ("chitu_diffusion.examples.flux2_klein_cp", ["--steps", "4"]),
        (
            "chitu_diffusion.serve.torchrun",
