...
def main(args: ServeConfig):
    if args.models.name != "Qwen-Image":
        raise ValueError(f"test/test_qwen_image.py expects models=Qwen-Image, got {args.models.name}.")
    ...
    try:
        ...
        if rank == 0:
            ...
            logger.info("Qwen-Image request: %s", reqs)
        else:
            reqs = []
        ...
        if rank == 0:
            logger.info("Qwen-Image time cost %.3fs", elapsed_s)
