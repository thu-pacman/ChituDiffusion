...
from chitu_diffusion.parallel import UspTopology, create_ulysses_transport
...
def _topology(world_size: int, rank: int, transport) -> UspTopology:
    return UspTopology(
        lane_ranks=tuple(range(world_size)),
        ulysses_ranks=tuple(range(world_size)),
        ring_ranks=(rank,),
        ulysses_rank=rank,
        ring_rank=0,
        ulysses_degree=world_size,
        ring_degree=1,
        ulysses_process_group=dist.group.WORLD,
    ...
    try:
        methods = (
            (
                "agkv",
                WanCpAttnProcessor(
                    SimpleNamespace(active=active, active_usp=None),
                    mode="agkv",
                ),
                None,
            ),
            (
                ...
                WanCpAttnProcessor(
                    SimpleNamespace(
                        active=active,
                        active_usp=_topology(world_size, rank, torch_transport),
                    ),
                    mode="usp",
                ),
                None,
            ...
            (
                ...
                WanCpAttnProcessor(
                    SimpleNamespace(
                        active=active,
                        active_usp=_topology(world_size, rank, fast_transport),
                    ),
                    mode="usp",
                ),
                "0",
