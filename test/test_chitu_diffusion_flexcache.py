...
class _ToyTransformer(nn.Module):
    def __init__(
        ...
        super().__init__()
        setattr(self, group_name, nn.ModuleList([_Block(attention_name, feedforward_name)]))
...
def test_meancache_matches_official_sigma_jvp_trajectory() -> None:
    ...
    for index in range(50):
        ...
        if index in strategy.fresh_indices:
            ...
            interval = (
                state.scheduler.sigmas[index + 1]
                - state.scheduler.sigmas[index]
            )
...
def test_cache_cli_constructs_only_the_selected_strategy() -> None:
    ...
    args = parser.parse_args(
        ["--cache-strategy", "none", "--pab-self-interval", "0"]
    )
