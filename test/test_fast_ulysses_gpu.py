...
def test_torch_usp_hybrid_ring_matches_full_attention() -> None:
    if (
...
@pytest.mark.gpu
@pytest.mark.distributed
@pytest.mark.parametrize(("heads", "expected_u"), [(12, 4), (6, 2)])
def test_fast_ulysses_outer_ring_matches_full_attention(
    heads: int,
    expected_u: int,
) -> None:
    if (
        ...
        pytest.skip("run with torchrun on exactly eight GPUs")
    rank = dist.get_rank()
    device = torch.device("cuda", int(os.environ["LOCAL_RANK"]))
    generator = torch.Generator(device=device).manual_seed(20260819 + heads)
    query = torch.randn(
        1, 256, heads, 64, device=device, dtype=torch.bfloat16, generator=generator
    )
    key = torch.randn(
        1, 256, heads, 64, device=device, dtype=torch.bfloat16, generator=generator
    )
    ...
    parallel = EpeParallelContext.from_torchrun(
        ...
        owns_process_group=False,
        ulysses_degree=None,
        ulysses_transport="fast_ulysses",
    )
    try:
        topology = parallel.active_usp_for_heads(heads)
        assert topology.ulysses_degree == expected_u
        assert topology.ring_degree == 8 // expected_u
        output = ImageSelfAttention("usp")(
            query[:, start:stop],
            key[:, start:stop],
            value[:, start:stop],
            lane_process_group=parallel.active.process_group,
            usp_topology=topology,
        )
        reference = F.scaled_dot_product_attention(
            query.transpose(1, 2),
            key.transpose(1, 2),
