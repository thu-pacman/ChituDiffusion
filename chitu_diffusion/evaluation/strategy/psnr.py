...
from chitu_diffusion.evaluation.utils.distributed import get_rank
from chitu_diffusion.evaluation.utils.reference_metrics import align_video_pair, load_video_frames

logger = getLogger(__name__)


class PsnrStrategy(ReferenceMetricStrategy):
    ...
    def evaluate(self, payload=None, args=None, max_frames: int = 16, **kwargs):
        ...
        for pair in pairs:
            ...
            for gen_frame, ref_frame in zip(gen_frames, ref_frames):
                score = peak_signal_noise_ratio(ref_frame, gen_frame, data_range=255.0)
        ...
        logger.info("PSNR eval done: %s", result_path)
        return result
