...
class _FastCERingSession:
    ...
    def acquire(self, phase_index: int) -> RingKVPhase:
        ...
        if phase_index != self._next_acquire or phase_index != self._next_release:
            raise RuntimeError(
                "Flash Ring phases must be acquired and released in order"
            )
        if phase_index == 0:
...
class FastRingKVTransport:
    ...
    def __init__(
        ...
        *,
        ...
        self._epoch = 0
        self._max_epoch = ((2**31 - 1) - (world_size - 1)) // (world_size - 1) + 1
            ...
            for phase_index in range(session.phase_count):
                ...
                try:
                    ...
                    if (
                        ...
                        raise RuntimeError(
                            "transport returned an incompatible K/V shard"
                        )
