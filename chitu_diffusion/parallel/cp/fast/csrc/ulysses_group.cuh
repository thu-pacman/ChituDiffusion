...
// CE transfer path: pitched cudaMemcpy2DAsync fan-out over the CE streams the schedule
// asks for, joined back to `stream` with events. The caller appends the flag barrier (no
// nvshmem quiet needed: these are not NVSHMEM proxy writes). Rationale:
// all_to_all_ce.cu file header; CEResources/CESchedule live in ce_schedule.cuh.
...
public:
    ...
    // Best CE fan-out for one transfer shape, measured once and cached. Which
    // schedule wins is a property of the host's interconnect -- a PCIe GPU
    // shares one egress port across peers while an NVLink GPU has a link per
    // peer -- so it is timed rather than assumed (rationale: ce_schedule.cuh).
    // Same hang-safety contract as resolve_config: under SPMD every rank misses
    // the same key together and runs an identical candidate list, so the
    // barriers inside the micro-benchmark stay in lockstep.
    const CESchedule& resolve_ce_schedule(const CEScheduleKey&                key,
                                          const std::function<CESchedule()>& tune);

    // NUMA layout of the group's devices, resolved once from any symmetric
    // buffer's peer pointers (every buffer reports the same devices).
    const CENumaLayout& ce_numa_layout(const std::vector<uint64_t>& peer_ptrs);
...
private:
    ...
    // Best CE fan-out per transfer shape (see resolve_ce_schedule). Lock-free
    // std::map, serial access only, like the config caches above.
    std::map<CEScheduleKey, CESchedule> ce_schedule_cache_;
    CENumaLayout                        ce_numa_layout_;
    bool                                ce_numa_ready_ = false;
