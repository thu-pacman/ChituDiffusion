...
const CENumaLayout& UlyssesGroup::ce_numa_layout(const std::vector<uint64_t>& peer_ptrs)
{
    if (!ce_numa_ready_) {
        ce_numa_layout_ = resolve_ce_numa_layout(my_rank_, peer_ptrs);
        ce_numa_ready_  = true;
    }
    return ce_numa_layout_;
}

const CESchedule& UlyssesGroup::resolve_ce_schedule(const CEScheduleKey&               key,
                                                    const std::function<CESchedule()>& tune)
{
    auto it = ce_schedule_cache_.find(key);
    if (it == ce_schedule_cache_.end())
        it = ce_schedule_cache_.emplace(key, tune()).first;
    return it->second;
}
