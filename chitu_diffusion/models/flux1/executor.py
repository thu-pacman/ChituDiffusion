...
class Flux1ImageDecoderExecutor(DiffusersBackend):
        ...
        for height, width in resolutions:
            ...
            for lane_width in self.parallel_context.allowed_widths:
                ...
                generator = torch.Generator(device=device).manual_seed(
                    5_000_011 + height * 97 + width * 193 + lane[0] * 997
                )
                latents = torch.randn(
    ...
    def normalize_request(self, request: Any) -> Flux1Request:
        if isinstance(request, Flux1Request):
            return request
        if hasattr(request, "model_dump"):
            request = request.model_dump()
        if not isinstance(request, Mapping):
            raise TypeError("Flux.1 requests must be Flux1Request or a mapping")
        return Flux1Request(
            prompt=request.get("prompt"),
            prompt_2=request.get("prompt_2"),
    ...
    def prepare_request(self, request: Any) -> Flux1DenoiseState:
        normalized = self.normalize_request(request)
        device = (
