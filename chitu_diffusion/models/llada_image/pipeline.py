...
from .diffusers_components import (
...
class LLaDAImageDiffusionPipeline(LLaDAImageRuntimeMixin, DiffusionPipeline):
    r"""
    ...
    def from_pretrained(
        ...
        *,
        ...
        parallel_context: EpeParallelContext | None = None,
        allowed_lane_widths: tuple[int, ...] | None = None,
        attention_mode: str = "agkv",
        ulysses_degree: int | None = None,
        ulysses_transport: str | None = None,
        agkv_transport: str | None = None,
        cfg_parallel: bool = True,
        epe_options: dict[str, Any] | None = None,
    ) -> "LLaDAImageDiffusionPipeline":
        root = Path(pretrained_model_name_or_path).expanduser()
        if not root.is_dir():
            raise ValueError(
                "LLaDA-Image requires a local Diffusers checkpoint directory; "
                f"not found: {root}"
            )
        ...
        if parallel_context is None:
            parallel_context = EpeParallelContext.from_torchrun(
                allowed_widths=allowed_lane_widths,
                ulysses_degree=ulysses_degree,
                ulysses_transport=ulysses_transport,
                agkv_transport=agkv_transport,
            )
        resolved_epe_options = dict(epe_options or {})
        resolved_epe_options.setdefault("attention_mode", attention_mode)
        resolved_epe_options.setdefault("cfg_parallel", cfg_parallel)
        epe = LLaDAImageEpeModule(
            parallel_context,
            **resolved_epe_options,
        )
        ...
        vae = AutoencoderKLFlux2.from_pretrained(root / "vae", **load_kwargs)
        ...
        return cls(
            ...
            queryformer=LLaDAImageQueryFormerModel.from_pretrained(
                root / "queryformer", **load_kwargs
            ),
            text_projection=LLaDAImageTextProjectionModel.from_pretrained(
                root / "text_projection",
                **load_kwargs,
            ),
            sigvq=LLaDAImageSigVQModel.from_pretrained(root / "sigvq", **load_kwargs),
            transformer=EpeLLaDAImageTransformer2DModel.from_pretrained(
                root / "transformer",
                epe_module=epe,
                **load_kwargs,
            ),
    ...
    def _encode_text(
        ...
        query_embeds = self.queryformer(
            inputs_embeds.to(
                device=self.queryformer.device, dtype=self.queryformer.dtype
            ),
        ...
        attention_mask = torch.cat(
            [
                attention_mask,
                attention_mask.new_ones(attention_mask.shape[0], query_embeds.shape[1]),
            ],
            dim=1,
        )
        ...
        backbone_attention_mask = attention_mask[:, None, None, :].expand(
        ...
        backbone_attention_mask = torch.where(
            ...
            torch.full(
                (), mask_value, dtype=inputs_embeds.dtype, device=text_encoder_device
            ),
        )
        ...
        prompt_embeds = self.text_projection(
            hidden_states.to(
                device=self.text_projection.device, dtype=self.text_projection.dtype
            )
        ).hidden_states
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            prompt_embeds, prompt_attention_mask = self._encode_text(
    ...
    def generate_vq_tokens(
        ...
        for prompt in prompts:
            ...
            uncond_prompt = f"<role>SYSTEM</role> {system_prompt} <role>HUMAN</role><uncondition><role>ASSISTANT</role>"
            ...
            output_ids = self.text_encoder.generate_bd_image_logic(
                data={
                    "input_ids": torch.tensor(
                        input_ids, device=self.text_encoder.device
                    ).unsqueeze(0),
            ...
            token_ids = (
                output_ids[0, len(input_ids) : len(input_ids) + image_token_count]
                - image_token_offset
            )
            if len(token_ids) != image_token_count:
                raise ValueError(
                    f"The MLLM generated {len(token_ids)} VQ tokens, expected {image_token_count}."
                )
            if torch.any(
                (token_ids < 0) | (token_ids >= self.sigvq.config.codebook_size)
            ):
                raise ValueError(
                    "The MLLM generated token IDs outside the SigVQ codebook."
                )
    ...
    def check_inputs(
        ...
        if generation_mode in {"text", "vq"} and image is not None:
            raise ValueError(
                f"`image` must be omitted when `generation_mode='{generation_mode}'`."
            )
        ...
        required_multiple = self.latent_scale_factor * (
            2 if generation_mode == "editing" else 1
        )
        if (
            height <= 0
            or width <= 0
            or height % required_multiple != 0
            or width % required_multiple != 0
        ):
            raise ValueError(
                f"`height` and `width` must be divisible by {required_multiple}."
            )
        ...
        if prompt_embeds is not None and prompt_attention_mask is None:
            raise ValueError(
                "`prompt_attention_mask` is required with `prompt_embeds`."
            )
        if (
            ...
            raise ValueError(
                "`negative_prompt_attention_mask` is required with `negative_prompt_embeds`."
            )
        if num_images_per_prompt < 1:
            raise ValueError("`num_images_per_prompt` must be at least 1.")
        if not all(
            name in self._callback_tensor_inputs
    ...
    def _encode_source_image(
        ...
        if image.shape[0] != batch_size:
            raise ValueError(
                f"The image batch size must be 1 or {batch_size}, but is {image.shape[0]}."
            )
    ...
    def __call__(
        ...
        The default timestep schedule matches the source checkpoint configuration: 20 first-order flow-matching steps,
        Kumaraswamy parameters `(1.17, 0.8, 1.1)`, and boundary gaps `(0.001, 0.0)`.
        ...
        if latents is None:
            ...
            if latents.shape != latent_shape:
                raise ValueError(
                    f"Expected `latents` to have shape {latent_shape}, got {tuple(latents.shape)}."
                )
        ...
        if do_classifier_free_guidance:
            uncond_cap_feats = [
                ...
                for embeds, mask in zip(
                    negative_prompt_embeds, negative_prompt_attention_mask.bool()
                )
            ]
        ...
        if semantic_features is not None:
            ...
            if do_classifier_free_guidance:
                ...
                ).to(device=self.transformer.device, dtype=self.transformer.dtype)
            ...
            for step_index, timestep in enumerate(timesteps):
                ...
                if callback_on_step_end is not None:
                    ...
                    callback_outputs = callback_on_step_end(
                        self, step_index, timestep, callback_kwargs
                    )
