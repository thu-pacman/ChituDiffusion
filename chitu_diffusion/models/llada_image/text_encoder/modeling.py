...
from ..text_encoder_ops import fused_moe_forward
from .configuration import LLaDA2MoeConfig
...
class LLaDA2MoePreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, LLaDA2MoeExperts):
...
class LLaDA2MoeRotaryEmbedding(nn.Module):
    def __init__(self, config: LLaDA2MoeConfig, device=None):
        ...
        self.config = config
        self.rope_init_fn = (
            self._compute_default_rope_parameters
...
class LLaDA2MoeDecoderLayer(nn.Module):
    def __init__(self, config: LLaDA2MoeConfig, layer_idx: int):
        ...
        try:
            attention_class = ATTENTION_CLASSES[attention_implementation]
        except KeyError as exc:
            raise ValueError(
                "LLaDA2 text encoder supports only the 'eager' and 'sdpa' "
                f"attention implementations, got {attention_implementation!r}"
            ) from exc
        self.attention = attention_class(config=config, layer_idx=layer_idx)
...
class LLaDA2MoeModelLM(LLaDA2MoePreTrainedModel, GenerationMixin):
    ...
    def from_local_pretrained(
        ...
        *,
        ...
        """Load a local safetensors checkpoint without remote model code.

        Transformers 5 can silently leave nested ``PreTrainedModel`` linear
        layers initialized when this released model is loaded directly through
        ``from_pretrained``. Building on ``meta`` and assigning every released
        tensor also keeps peak host memory bounded to one checkpoint shard.
        """
        ...
        if checkpoint_keys != expected_keys:
            ...
            raise RuntimeError(
                "LLaDA text checkpoint keys do not match the local model: "
        ...
        for shard_path in shard_paths:
            ...
            if torch_dtype is not None:
                state_dict = {
                    name: value.to(torch_dtype) if value.is_floating_point() else value
            ...
            if incompatible.unexpected_keys:
                raise RuntimeError(
                    f"Unexpected keys in {shard_path}: "
                    f"{incompatible.unexpected_keys[:8]}"
                )

        for module in model.modules():
            if isinstance(module, LLaDA2MoeRotaryEmbedding):
                inv_freq, attention_scaling = module.rope_init_fn(
                    module.config, torch.device("cpu")
                )
        ...
        if meta_tensors:
            raise RuntimeError(
                f"LLaDA text checkpoint left tensors on meta: {meta_tensors[:8]}"
            )
        model.eval()
        return model
    ...
    def _get_num_transfer_tokens(block_length, steps):
        ...
        return tuple(base + (step_index < remainder) for step_index in range(steps))

    @staticmethod
    def _select_transfer_tokens(
        active: torch.Tensor,
        ...
        *,
        ...
        """Select the scheduled and high-confidence tokens without a host read."""
        scores = torch.where(active, confidence, -torch.inf)
        high_confidence = scores > threshold
        selected = torch.zeros_like(active)
        if count > 0:
            _, indices = torch.topk(scores, k=min(count, scores.shape[-1]), dim=-1)
            selected.scatter_(-1, indices, True)
        return (selected | high_confidence) & active

    @staticmethod
    def _completed_stop_offset(
        tokens: torch.Tensor,
        *,
        stop_token: int,
        mask_token: int,
    ) -> int:
        """Return the first stop token with a fully generated prefix."""
        offsets = torch.arange(tokens.numel(), device=tokens.device)
        unresolved_prefix = (tokens == mask_token).cumsum(dim=0)
        sentinel = torch.full_like(offsets, tokens.numel())
        completed_stops = torch.where(
            (tokens == stop_token) & (unresolved_prefix == 0), offsets, sentinel
        )
        return int(completed_stops.min().item())
    ...
    def generate_bd_image_logic(
        ...
        if use_cfg:
            ...
            if torch.is_tensor(uncond_ids):
                uncond_ids = uncond_ids.flatten().to(
                    device=self.device, dtype=torch.long
                )
            else:
                uncond_ids = torch.tensor(
                    uncond_ids, device=self.device, dtype=torch.long
                )
            if uncond_ids.numel() == 0:
                raise ValueError("The unconditional prompt must not be empty")
            pad_len = prompt_length - uncond_ids.numel()
            if pad_len < 0:
                raise ValueError(
            ...
            uncond_input[0, -uncond_ids.numel() :] = uncond_ids
        ...
        for block_index in range(prefill_blocks, num_blocks):
            ...
            for step_index in range(steps):
                ...
                selected = self._select_transfer_tokens(
                    active,
                    confidence,
            ...
            if eos_early_stop:
                ...
                stop_offset = self._completed_stop_offset(
                    current[0, prompt_length:],
                    stop_token=stop_token,
                    mask_token=mask_id,
                )
                if stop_offset < current.shape[1] - prompt_length:
                    return x[:, : prompt_length + stop_offset + 1]
