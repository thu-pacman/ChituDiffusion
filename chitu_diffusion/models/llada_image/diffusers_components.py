    ...
    def __call__(
        ...
        if freqs_cis is not None:
            with torch.autocast(device_type=hidden_states.device.type, enabled=False):
                ...
                query = (
                    torch.view_as_real(query_complex * frequencies)
                    .flatten(3)
                    .to(dtype=query.dtype)
                )
                key = (
                    torch.view_as_real(key_complex * frequencies)
                    .flatten(3)
                    .to(dtype=key.dtype)
                )
...
class LLaDAImageAttention(nn.Module, AttentionModuleMixin):
    ...
    def __init__(self, dim: int, num_heads: int, norm_eps: float, qk_norm: bool):
        ...
        self.norm_q = (
            RMSNorm(self.head_dim, eps=norm_eps, elementwise_affine=False)
            if qk_norm
            else None
        )
        self.norm_k = (
...
class LLaDAImageTransformerBlock(nn.Module):
    def __init__(
        self, dim: int, num_heads: int, norm_eps: float, qk_norm: bool, modulation: bool
    ):
        super().__init__()
...
class LLaDAImageTransformer2DModel(ModelMixin, ConfigMixin, AttentionMixin):
    ...
    def __init__(
        ...
        self.noise_refiner = nn.ModuleList(
            [
                LLaDAImageTransformerBlock(
                    dim, n_heads, norm_eps, qk_norm, modulation=True
                )
        ...
        self.context_refiner = nn.ModuleList(
            [
                LLaDAImageTransformerBlock(
                    dim, n_heads, norm_eps, qk_norm, modulation=False
                )
                for _ in range(n_refiner_layers)
            ]
        )
