...
class Flux2ChituAttnProcessor(Flux2AttnProcessor):
    """Flux2 joint attention processor backed by Chitu's selectable attention backend."""
...
class Flux2ChituParallelSelfAttnProcessor(Flux2ParallelSelfAttnProcessor):
    """Flux2 single-stream self-attention processor backed by Chitu's selectable attention backend."""
...
class Flux2SingleTransformerBlock(nn.Module):
    def __init__(
        ...
        processor = (
            Flux2ChituParallelSelfAttnProcessor(attn_backend)
            if attn_backend is not None
            else Flux2ParallelSelfAttnProcessor()
        )
...
class Flux2TransformerBlock(nn.Module):
    def __init__(
        ...
        processor = Flux2ChituAttnProcessor(attn_backend) if attn_backend is not None else Flux2AttnProcessor()
...
class Flux2Transformer2DModel(
    ...
    def __init__(
        ...
        super().__init__()
        if model_type != "t2i":
            raise ValueError(f"Flux2-klein only supports t2i, got {model_type}.")
