...
def parallel_tiled_vae_decode(
    ...
    *,
    ...
    H = latents.shape[latent_split_dim]
    # Fall back to legacy rank-0-only decode when parallelism can't help or is off.
    if not parallel_decode_enabled() or world <= 1 or H < world:
        if global_rank != 0:
            return None
        return decode_fn(latents)

    base = (H + world - 1) // world  # ceil; every core is `base` rows (last may be shorter)

    core_start = rank_in_group * base
    core_end = min(core_start + base, H)
    # Decode this core plus an interior halo; the outer image border is left to the
    # VAE's own boundary handling (clamped), so border pixels match a full decode.
    lo = max(0, core_start - halo)
    hi = min(H, core_end + halo)
    ...
    for r in range(world):
        r_len = (min((r + 1) * base, H) - r * base) * scale
        if r_len <= 0:
            continue
        pieces.append(gathered[r].narrow(pixel_split_dim, 0, r_len))
