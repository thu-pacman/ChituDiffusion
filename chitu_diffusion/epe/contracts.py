...
@runtime_checkable
class DiffusionBackendProtocol(Protocol):
    """Model plugin shared by synchronous generation and the EPE runtime.
...
class DiffusionBackendFactory(Protocol):
    def build(self, context: ExecutorBuildContext) -> DiffusionBackendProtocol: ...
